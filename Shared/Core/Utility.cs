...
namespace Shared.Core
{
    ...
    {
        public static string PacketDebug(Type where, Packet packet)
        {
            return $"{@where.Name} [{Enum.GetName(@where, packet.Opcode) ?? $"0x{packet.Opcode.ToString("X4")}"}] " + $"\n{HexDump(packet.GetBytes())}";
        }
