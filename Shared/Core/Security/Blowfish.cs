...
namespace Shared.Core.Security
{
    internal class Blowfish
    {
        ...
        public Blowfish(byte[] keyPtr, int offset, int length)
        {
            _pArray = new uint[18];
            _sBoxes = new uint[4, 256];
            Initialize(keyPtr, offset, length);
        }
        ...
        public void Initialize(byte[] keyPtr, int offset, int length)
        {
            ...
            {
                temp[3] = keyPtr[j];
                temp[2] = keyPtr[(j + 1)%length];
                temp[1] = keyPtr[(j + 2)%length];
                temp[0] = keyPtr[(j + 3)%length];
                var data = BitConverter.ToUInt32(temp, 0);
                _pArray[i] ^= data;
                j = (j + 4)%(uint) length;
            }
