...
namespace Shared.Core.Security
{
    internal class Security
    {
        ...
        public void Recv(TransferBuffer raw_buffer)
        {
            ...
            lock (m_class_lock)
            {
                ...
                while (length > 0)
                {
                    ...
                    Buffer.BlockCopy(raw_buffer.Buffer, raw_buffer.Offset + index, m_recv_buffer.Buffer,
                ...
                if (incoming_buffers_tmp.Count > 0)
                {
                    foreach (var buffer in incoming_buffers_tmp)
                    {
                        ...
                        var packet_data = new Packet.PacketReader(buffer.Buffer);
        ...
        private static byte FromSecurityFlags(SecurityFlags flags)
        {
            return
                (byte)
                    (flags.none | flags.blowfish << 1 | flags.security_bytes << 2 | flags.handshake << 3 |
        ...
        private byte GenerateCheckByte(byte[] stream, int offset, int length)
        {
            ...
            return
                (byte)
                    (((checksum >> 24) & 0xFF) + ((checksum >> 8) & 0xFF) + ((checksum >> 16) & 0xFF) +
                     (checksum & 0xFF));
        }
        ...
        private void Handshake(ushort packet_opcode, Packet.PacketReader packet_data, bool packet_encrypted)
        {
            if (packet_encrypted)
            {
        ...
        private byte[] FormatPacket(ushort opcode, byte[] data, bool encrypted)
        {
            ...
            var writer = new Packet.PacketWriter();
        ...
        private KeyValuePair<TransferBuffer, Packet> GetPacketToSend()
        {
            ...
            if (packet.Massive)
            {
                ...
                var final = new Packet.PacketWriter();
                var final_data = new Packet.PacketWriter();

                var input_data = packet.GetBytes();
                var input_reader = new Packet.PacketReader(input_data);

                var workspace = new TransferBuffer(4089, 0, input_data.Length);

                while (workspace.Size > 0)
                {
                    var part_data = new Packet.PacketWriter();

                    var cur_size = workspace.Size > 4089 ? 4089 : workspace.Size;
                        // Max buffer size is 4kb for the client
                ...
                var final_header = new Packet.PacketWriter();
